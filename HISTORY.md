...
### Completed
- Migration `20260924150000_add_faq_categories.sql`, **applied to live Supabase** and verified. Purely additive:
  new `faq_categories` table (public read of active rows, admin-only writes via `current_user_is_admin()`;
  anon has SELECT only), and new nullable/defaulted columns on `faqs`: `category_id`, `slug`
  (auto-generated by `faqs_set_slug` trigger from the question), `is_featured`. No existing FAQ row was
  deleted or recreated and the existing `faqs` RLS policies were not touched.
- Seeded 4 categories (General Questions, Pricing & Payments, Process & Delivery, Support & Technical);
  the existing 14 FAQs were filed into them by id and the first 6 (previous home-page behaviour) marked
  featured. Verified as the `anon` role: 4 categories and 14 FAQs readable.
- New page `/faqs` (`src/routes/faqs.tsx`, `src/components/site/FaqExplorer.tsx`): loader fetches all data
  on the server so every question and answer is in the HTML. Default view is "All Questions" grouped by
  category (h2 per category, h3 per question). Category filtering is `?category=<slug>` via real links.
  Answers stay in the DOM when collapsed (CSS grid-rows 0fr/1fr, not unmounted/hidden). Deep links
  `/faqs#<question-slug>` open and scroll to that answer. FAQPage JSON-LD is built from the real rows
  (`<` escaped) plus BreadcrumbList. Colours use the existing theme tokens (`--electric`, `--primary`).
  If the table is empty/unreachable, the built-in DEFAULT_FAQS (the old 14 fallbacks) are shown.
- Home / pricing `FaqSection`: shows only featured FAQs (falls back to the first 6) and a
  "View all FAQs" button linking to `/faqs`.
- Navbar: `nav_links` row (FAQ, `/faqs`, sort 45) inserted by the migration, plus FALLBACK entry and
  `nav.faq` in all 8 locales. Footer FAQ link now points to `/faqs`. `/faqs` added to the sitemap.
- Admin (Web Portal -> FAQ tab): now has Questions and Categories sub-tabs. Questions: category select,
  question, answer, sort order, "Show on the home page", active toggle. Categories: name, slug, description,
  sort order, active toggle. Hiding a category hides its questions publicly; deleting one leaves its
  questions in place (shown under "More Questions").
- Pure helpers live in `src/lib/faq-utils.ts` (unit-tested in `test/faq/faq.test.ts`); data access in `src/lib/faq.ts`.
- Checks: `npm run typecheck` 0 errors, `npm run lint` 0 errors / 11 baseline warnings, `npm test` 7/7 pass,
  `npm run build` OK.

### Commit
- See `git log` for the commit right after `a4d0319`. Pushed to `origin/main`.

### Notes
- Local SSR check under `wrangler dev --local` confirmed all 14 answers, h3 headings, aria attributes and
  FAQPage JSON-LD in the raw HTML, but the sandbox cannot reach Supabase, so that run used the built-in
  fallback data. Live category grouping was verified via SQL as `anon`, not via a browser. Open
  `/faqs` on the deployed site once to eyeball the layout and the four category tabs.
- Deliberately NOT done (per owner): no search box, no `/faq.md`, no `llms.txt` changes. AI-specific text
  files will be handled separately for the whole site.
- The owner wrote both `/faqs` and `/faq`; `/faqs` is used everywhere.
