  ...
  return (
    <div className={cn("flex items-center", className)}>
      <img
        ...
        className={cn(h, "w-auto object-contain block dark:hidden")}
      />
      <img
        src={logoDark}
        alt="ELFO Innovations"
        className={cn(h, "w-auto object-contain hidden dark:block")}
      />
    </div>
  );
}
...
export function ElfoLogoMark({ className }: { className?: string }) {
  ...
  if (override) {
    return (
      <img src={override} alt="ELFO Innovations" className={cn("object-contain", className)} />
    );
  }
  return (
    <>
