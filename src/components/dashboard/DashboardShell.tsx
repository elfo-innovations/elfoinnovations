  ...
  return (
    <>
      <nav className="flex-1 space-y-0.5 px-3 py-4">
        ...
        {nav.map((n) => {
          const active =
            n.to === "/"
              ? pathname === "/"
              : pathname === n.to || (n.to !== `/${role}` && pathname.startsWith(n.to));
          return (
            <Link
              key={n.to}
              to={n.to}
      ...
      <div className="space-y-2 border-t p-3">
        ...
        <button
          onClick={onSignOut}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground"
        >
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4 text-center">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your account doesn't have {role} access yet.
          </p>
          <Button
...
function ShellInner({
  ...
  const { data: clientRow } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("clients").select("closed_at").eq("user_id", user.id).maybeSingle())
        .data,
  });
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background px-4 text-center">
        <div className="max-w-md">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Your project has been delivered and this client portal has been closed by Elfo
            Innovations. Please contact us if you need access again.
          </p>
          <Button
            className="mt-6 rounded-full"
            onClick={async () => {
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 items-center justify-between gap-2 border-b bg-background/60 px-4 backdrop-blur sm:px-6">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="hidden text-xs font-semibold uppercase tracking-widest text-muted-foreground lg:inline">
              {role} portal
            </span>
          </div>
