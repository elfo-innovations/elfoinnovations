        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {offers.map((o) => (
            <div key={o.id} className="glass-card overflow-hidden rounded-2xl">
              {o.banner_image_url && (
                <img src={o.banner_image_url} alt={o.title} className="h-40 w-full object-cover" />
              )}
              <div className="p-6">
                {o.discount && (
                ...
                {o.end_date && (
                  <div className="mt-3 text-xs text-muted-foreground">
                    Ends {new Date(o.end_date).toLocaleDateString()}
                  </div>
                )}
                {o.cta_label && o.cta_href && (
                  <Link to={o.cta_href}>
                    <Button className="mt-5 w-full rounded-full">{o.cta_label}</Button>
                  </Link>
                )}
              </div>
            </div>
