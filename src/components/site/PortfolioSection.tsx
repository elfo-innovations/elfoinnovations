        ...
        {items.length > 0 && (
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((p) => (
              <article
                key={p.id}
                className="glass-card overflow-hidden rounded-3xl transition-all hover:electric-glow"
              >
                <div className="aspect-[16/10] overflow-hidden border-b bg-muted">
                  {p.after_image_url || p.before_image_url ? (
                    <img
                      src={(p.after_image_url || p.before_image_url)!}
                      alt={p.project_name}
                      className="h-full w-full object-cover"
                    />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center circuit-pattern text-xs uppercase tracking-widest text-muted-foreground">
                      ELFO Project
                    </div>
                  )}
                </div>
                <div className="p-5">
                  ...
                  {p.client_name && (
                    <div className="text-xs text-muted-foreground">for {p.client_name}</div>
                  )}
                  {p.description && (
                  ...
                  {p.technologies?.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1">
                      {p.technologies.slice(0, 4).map((t) => (
                        <span
                          key={t}
                          className="rounded-full border bg-card px-2 py-0.5 text-[10px]"
                        >
                          {t}
                        </span>
                      ))}
