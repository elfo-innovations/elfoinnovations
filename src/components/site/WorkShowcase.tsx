  ...
  return (
    <article className="glass-card group relative overflow-hidden rounded-3xl transition-all hover:-translate-y-1 hover:electric-glow">
      <div className="relative aspect-[16/10] overflow-hidden bg-muted">
        <img src={data.image_url} alt={data.title || ""} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.05]" />
...
export function WorkShowcase() {
  const { data: projects } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("portfolio_projects").select("*").eq("is_active", true).order("sort_order")).data ?? [],
  });
  ...
  const { data: sideBanners } = useQuery({
    ...
    queryFn: async () => {
      const [left, right] = await Promise.all([
        supabase.from("promo_banners").select("*").eq("position", "work_left").eq("is_active", true).order("sort_order").limit(1),
        supabase.from("promo_banners").select("*").eq("position", "work_right").eq("is_active", true).order("sort_order").limit(1),
      ]);
      return { left: left.data?.[0] as PromoCard | undefined, right: right.data?.[0] as PromoCard | undefined };
    },
  });

  const works: Work[] =
    projects && projects.length > 0
      ? projects.map((p: any) => ({ title: p.project_name, tag: p.category || "", image: p.after_image_url, repo: p.github_url || null }))
      : DEFAULT_WORKS;
