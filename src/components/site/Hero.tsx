...
function Monitor3DPlaceholder() {
  return (
    <div
      className="relative mx-auto flex w-full max-w-[820px] items-center justify-center"
...
export function Hero() {
  ...
  const pattern = highlights.length
    ? new RegExp(`(${highlights.map(escape).join("|")})`, "gi")
    : null;
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-4 pb-20 pt-16 sm:px-6 lg:grid-cols-2 lg:gap-8 lg:px-8 lg:pt-24">
        <div className="max-w-2xl">
          ...
          >
            {segments.map((seg, i) =>
              pattern && highlights.some((hl) => hl.toLowerCase() === seg.toLowerCase()) ? (
                <span key={i} className="electric-text">
                  {seg}
                </span>
              ) : (
                <span key={i}>{seg}</span>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Button
              size="lg"
              onClick={(h.primary_cta_action ?? "inquiry") === "inquiry" ? open : undefined}
              className="rounded-full px-7 electric-glow"
            >
            ...
            {(data ? h.secondary_cta_label : true) && (
              <Link to={h.secondary_cta_href || "/portfolio"}>
                <Button size="lg" variant="outline" className="rounded-full px-7">
                  {h.secondary_cta_label || "See our work"}
                </Button>
              </Link>
            )}
          </div>
        ...
        <div className="relative">
          ...
          <div className="pointer-events-none absolute inset-0 -z-10">
            ...
            >
              ...
              <path
                d="M 20 380 L 120 380 L 180 320 L 260 320 L 320 260 L 400 260"
                stroke="url(#circuitLine)"
                strokeWidth="1.5"
              />
              <path
                d="M 0 340 L 80 340 L 140 280 L 240 280 L 300 220 L 400 220"
                stroke="url(#circuitLine)"
                strokeWidth="1"
              />
              <path
                d="M 40 400 L 40 320 L 100 260 L 100 180 L 160 120 L 260 120"
                stroke="url(#circuitLine)"
                strokeWidth="1"
              />
              <path
                d="M 200 400 L 200 340 L 280 260 L 280 180 L 360 100"
                stroke="url(#circuitLine)"
                strokeWidth="1.5"
              />
