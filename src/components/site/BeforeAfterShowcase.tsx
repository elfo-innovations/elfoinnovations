...
const placeholderShot = (label: string, tint: "muted" | "bright" = "muted") => (
  <div
    className={`relative flex h-full w-full items-center justify-center overflow-hidden ${tint === "bright" ? "bg-[var(--gradient-primary)]" : "bg-muted"}`}
  >
    <div className="absolute inset-0 opacity-30 circuit-pattern" />
    <div className="relative text-center">
      <div
        className={`text-[10px] sm:text-xs font-semibold uppercase tracking-widest ${tint === "bright" ? "text-white/80" : "text-muted-foreground"}`}
      >
        {label}
      </div>
    </div>
    ...
    <section className="border-t bg-background py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            Every ELFO INNOVATIONS delivery is engineered for measurable improvement — in speed,
      ...
      >
        ...
        <div
          ref={scrollerRef}
          dir="ltr"
          className="overflow-x-auto no-scrollbar px-4 sm:px-8 lg:px-12"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] items-center gap-3">
        <div className="relative aspect-[16/10] overflow-hidden rounded-2xl border bg-muted">
          {before ? (
            ...
            placeholderShot("Before")
          )}
          <span className="absolute left-2 top-2 rounded-full bg-background/80 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider backdrop-blur">
            Before
          </span>
        </div>
        ...
        <div className="relative aspect-[16/10] overflow-hidden rounded-2xl border border-primary/30 bg-muted">
          {after ? (
            ...
            placeholderShot("After", "bright")
          )}
          <span className="absolute left-2 top-2 rounded-full bg-primary/90 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-primary-foreground backdrop-blur">
            After
          </span>
        </div>
      </div>
