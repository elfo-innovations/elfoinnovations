  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Star
            className={`h-6 w-6 ${n <= value ? "fill-primary text-primary" : "text-muted-foreground/40"}`}
          />
        </button>
      ))}
    </div>
...
function ClientReviews() {
  ...
  const { data: projects } = useQuery({
    ...
    enabled: !!client,
    queryFn: async () =>
      (await supabase.from("projects").select("id,name").eq("client_id", client!.id)).data ?? [],
  });
  ...
  const submit = async () => {
    ...
    if (error) {
      if (error.code === "23505")
        toast.error("You already have an active review for this project.");
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Share your experience working with Elfo Innovations.
          </p>
        </div>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div>
              ...
              <Textarea
                ...
                rows={5}
                maxLength={1200}
                placeholder="Tell us about the process, quality, and outcome…"
              />
            </div>
            {(projects?.length ?? 0) > 0 && (
              <div>
            ...
            <label className="flex items-start gap-2 rounded-xl border bg-muted/30 p-3 text-sm">
              <Checkbox
                checked={allowPublic}
                onCheckedChange={(v) => setAllowPublic(!!v)}
                className="mt-0.5"
              />
              <span>
                I agree that Elfo Innovations may display this review publicly on its website.
              </span>
            </label>
