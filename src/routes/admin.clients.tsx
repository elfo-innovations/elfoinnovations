  ...
  (data ?? []).forEach((c) => {
    if (c.referred_by_client_id) {
      referredCountByReferrer.set(
        ...
        (referredCountByReferrer.get(c.referred_by_client_id) ?? 0) + 1,
      );
    }
  });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Create client portal access and share credentials.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(data ?? [])
          .filter((c) => {
            ...
            if (
              q &&
              ![c.full_name, c.email, c.phone, c.company, c.country].some((v) =>
              ...
              <div key={c.id} className="glass-card rounded-2xl p-5">
                ...
                {referrer && (
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-medium text-amber-500">
                    <Link2 className="h-3 w-3 shrink-0" /> {c.full_name} used {referrer.full_name}'s
                    referral
                  </div>
                )}
                {referredCount > 0 && (
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-medium text-primary">
                    <Users className="h-3 w-3 shrink-0" /> Referred {referredCount} client
                    {referredCount > 1 ? "s" : ""}
                  </div>
                )}
                <div className="mt-3 flex flex-wrap justify-end gap-1">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      if (!confirm(`Delete ${c.full_name}? This removes their portal access.`))
                        return;
                      try {
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Share this password with the client after creation.
              </p>
            </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <KeyRound className="h-5 w-5 text-primary" /> Client Portal Credentials
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Share these with the client. This password won't be shown again — copy it now.
            </p>
