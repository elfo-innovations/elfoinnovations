  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("projects")
          .select("*, project_stages(*), clients(full_name)")
  ...
  return (
    <DashboardShell role="admin">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Click a project to manage stages, upload files, and assign developers.
      </p>
        ...
        {(data ?? []).map((p) => {
          ...
          return (
            <Link
              ...
              to="/admin/project/$id"
              params={{ id: p.id }}
              className="glass-card block rounded-2xl p-6 transition hover:border-primary"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    {p.clients?.full_name ?? "Unassigned client"}
                  </div>
                  {p.requirements && (
              ...
              <div className="mt-5">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-4">
                  {stages.map((s) => (
                    <div
                      key={s.id}
                      className={`rounded-xl border px-3 py-2 text-xs ${COMPLETED.has(s.status) ? "border-emerald-500 bg-emerald-500/10 text-emerald-600" : ["sent_to_client", "delivered", "admin_review"].includes(s.status) ? "border-amber-400 bg-amber-400/10 text-amber-600" : "border-border text-muted-foreground"}`}
                    >
