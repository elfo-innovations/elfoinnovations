  ...
  const { data: project } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("projects")
          .select("*, project_stages(*), project_files(*), clients(full_name)")
          .eq("id", id)
          .maybeSingle()
      ).data,
  });
  ...
  return (
    <DashboardShell role="developer">
          ...
          <div>
            ...
            {project.clients && (
              <div className="mt-2 text-xs text-muted-foreground">
                Client: <b>{project.clients.full_name}</b>
              </div>
            )}
      ...
      <p className="text-xs text-muted-foreground">
        Upload deliverables and mark stages ready for admin review. The admin forwards approved work
        to the client.
      </p>
            ...
            <div key={s.id} className="glass-card rounded-2xl p-5">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
                <Textarea
                  ...
                  value={comments[s.id] ?? s.developer_comment ?? ""}
                  onChange={(e) => setComments({ ...comments, [s.id]: e.target.value })}
                />
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => updateStage(s.id, { developer_comment: comments[s.id] ?? "" })}
                >
                  Save note
                </Button>
              </div>
