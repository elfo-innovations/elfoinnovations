...
function stageVisual(status: string) {
  ...
  if (["sent_to_client", "delivered", "admin_review", "revision_requested"].includes(status))
    return { color: "bg-amber-400", text: "text-amber-500", label: "Pending review", icon: Clock };
  ...
  const { data: project } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("projects")
          .select("*, project_stages(*), project_files(*)")
          .eq("id", id)
          .maybeSingle()
      ).data,
  });
  ...
  return (
    <DashboardShell role="client">
      ...
      <p className="text-xs text-muted-foreground">
        Approve each stage after reviewing what the team delivered.
      </p>
            ...
            <div key={stageName} className="glass-card rounded-2xl p-5">
              ...
              {s?.admin_comment && (
                <div className="mt-3 rounded-xl bg-muted/50 p-3 text-xs">
                  <b>Admin note:</b> {s.admin_comment}
                </div>
              )}
              {s?.client_comment && s?.status === "revision_requested" && (
                <div className="mt-2 rounded-xl bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-400">
                  <b>Your revision note:</b> {s.client_comment}
                </div>
              )}
              ...
              {rejectFor === s?.id && (
                <div className="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3">
                  ...
                  <div className="mt-2 flex justify-end gap-2">
                    ...
                    <Button size="sm" onClick={() => rejectStage(s!.id)}>
                      Send request
                    </Button>
                  </div>
                </div>
                ...
                <div className="mt-3 space-y-1.5">
                  {stageFiles.map((f: any) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between rounded-xl border bg-background p-2.5 text-xs transition hover:border-primary"
                    >
