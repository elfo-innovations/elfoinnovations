...
function stageVisual(status: string) {
  if (["client_approved", "admin_approved"].includes(status))
    return {
  ...
  if (["sent_to_client", "delivered", "admin_review", "revision_requested"].includes(status))
    return { color: "bg-amber-400", text: "text-amber-500", label: "In review", icon: Clock };
  return {
...
function AdminProjectDetail() {
    ...
    queryFn: async () =>
      (
        await supabase
          .from("projects")
          .select(
            "*, project_stages(*), project_files(*), clients(full_name, email), developers(full_name)",
          )
          .eq("id", id)
          .maybeSingle()
      ).data,
  ...
  const { data: devs } = useQuery({
    queryKey: ["all-devs"],
    queryFn: async () =>
      (await supabase.from("developers").select("id, full_name, status").order("full_name")).data ??
      [],
  });
  ...
  const assignDev = async (developer_id: string) => {
    const { error } = await supabase
      .from("projects")
      .update({ developer_id: developer_id || null })
      .eq("id", id);
    ...
    if (project?.client_id) {
      ...
      if (!existing) {
        await supabase
          .from("conversations")
          .insert({ client_id: project.client_id, project_id: id, subject: project.name });
      }
    }
  ...
  const closeProject = async () => {
    ...
    if (
      !confirm(
        "Close this project? The client's portal access will be revoked. All records stay in your admin CRM.",
      )
    )
      return;
    setClosing(true);
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {project.closed_at && (
              <Badge variant="outline" className="border-emerald-500 text-emerald-600">
                Closed · {new Date(project.closed_at).toLocaleDateString()}
              </Badge>
            )}
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          ...
          {project.developers && (
            <div className="text-xs text-muted-foreground">
              Currently: <b>{project.developers.full_name}</b>
            </div>
          )}
        </div>
          ...
          if (project.closed_at) {
            return (
              <div className="mt-5 rounded-xl border border-emerald-500/40 bg-emerald-500/5 p-3 text-xs text-emerald-700 dark:text-emerald-400">
                Project has been closed. Client portal access is revoked. Records remain in your
                admin CRM.
              </div>
            );
          }
            ...
            <div key={s.id} className="glass-card rounded-2xl p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-xl ${v.color} text-white`}
                  >
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => updateStage(s.id, { admin_comment: comments[s.id] ?? "" })}
                >
                  Save note
                </Button>
              </div>
                ...
                <div className="mt-3 space-y-1.5">
                  {stageFiles.map((f) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between rounded-xl border bg-background p-2.5 text-xs hover:border-primary"
                    >
