  ...
  return (
    <DashboardShell role="client">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Track your projects and stay in touch with your team.
      </p>
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <StatCard
          label="Active projects"
          value={stats?.active ?? "…"}
          hint="Submitted requirements"
          icon={Briefcase}
        />
        <StatCard
          label="Completed stages"
          value={stats?.completedStages ?? "…"}
          hint="Approved milestones"
          icon={CheckCircle2}
        />
        <StatCard
          label="Invoices"
      ...
      <div className="glass-card mt-8 rounded-2xl p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Every project goes through Frontend → Backend → Database → Hosting. Admin reviews each
          stage before it reaches you.
        </p>
      </div>
    </DashboardShell>
  );
