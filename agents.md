...
* **Finding 19 (test suite): the Finding 1 pricing trigger is tested with a real local Postgres integration test, not a copied-logic unit test.** `test/finding1/trigger.sql` is a verbatim `pg_get_functiondef()` pull of `generate_project_invoice()` — if that trigger is ever changed, re-pull it into that file rather than hand-editing it, or the test stops verifying real production behavior. The test runs against an isolated local PostgreSQL instance (see `HISTORY.md`, Finding 19 entry) — never against production Supabase, and Supabase branching is unavailable on this org's Free plan so don't attempt it again.
